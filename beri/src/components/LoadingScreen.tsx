 ...
 */
export function LoadingScreen({ loadingState, onRetry, isFirstVisit = false }: Props) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-habs-navy to-habs-navy-dark flex flex-col items-center justify-center p-6">
      <div className={`w-full ${isFirstVisit ? 'max-w-3xl' : 'max-w-md'}`}>
        <div className={`${isFirstVisit ? 'flex flex-col md:flex-row items-center md:items-start gap-8' : ''}`}>
          {/* Left column: logo + progress */}
          <div className={`text-center ${isFirstVisit ? 'md:flex-1' : ''}`}>
            {/* Logo */}
            <div className="mb-8">
              <img
                src={logoUrl}
          ...
          {/* Right column: How BERI Works (first visit only) */}
          {isFirstVisit && (
              ...
              <div className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-2xl p-6">
                ...
                <ul className="space-y-3 text-sm text-white/80">
                  ...
                  <li className="flex gap-3">
                    ...
                    <span><strong className="text-white">Thinks first</strong> &mdash; BERI reasons through retrieved information before composing a response.</span>
                  </li>
                  <li className="flex gap-3">
                    <span className="mt-0.5 text-habs-gold shrink-0">
